...
#### シンプルさを重視
- **1ファイル構成**: すべての機能を1つのHTMLファイルにまとめた構成を維持
...
#### 複雑な形式（ID付き）
```
ROHAN4600_0001:流(なが)し斬(ぎ)りが完全(かんぜん)に入(はい)れば、デバフの効果(こうか)が付与(ふよ)される。,ナガシギリガカンゼンニハイレバ、デバフノコウカガフヨサレル。
```

### テキスト解析の仕組み

#### 1. ID部分の処理
- `:`より前の部分（例：`ROHAN4600_0001`）は無視
- `:`より後の部分をテキストとして使用

#### 2. テキストの分割
- `,`で分割して2つの部分に分ける
- 前半：表示用テキスト（録音時に表示）
- 後半：正規化テキスト（LJSpeech形式で使用）

#### 3. 各エクスポート形式での処理

**Style-BERT-VITS2等・シンプル形式**
- 表示用テキストからルビ（括弧内の文字）を除去
- 例：`流(なが)し斬(ぎ)り` → `流し斬り`

**LJSpeech形式**
- 表示用テキストと正規化テキストの両方を使用
- ルビはそのまま保持
...
**録音時表示**：
```
流(なが)し斬(ぎ)りが完全(かんぜん)に入(はい)れば、デバフの効果(こうか)が付与(ふよ)される。
```

**Style-BERT-VITS2等出力**：
```
audio_0001.wav|test_speaker|JP|流し斬りが完全に入れば、デバフの効果が付与される。
```

**LJSpeech出力**：
```
audio_0001|流(なが)し斬(ぎ)りが完全(かんぜん)に入(はい)れば、デバフの効果(こうか)が付与(ふよ)される。|ナガシギリガカンゼンニハイレバ、デバフノコウカガフヨサレル。
```

### エッジケースの処理

- **`:`がない場合**：元のテキストをそのまま使用
- **`,`がない場合**：表示用テキスト = 正規化テキスト
- **ルビがない場合**：そのまま出力
